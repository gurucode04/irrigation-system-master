...
def DeviceControlUpdateView(request):
    obj = DeviceControl.objects.all()[0]
    if request.method == 'POST':
        obj.lower_threshold = request.POST['lower_threshold']
        obj.upper_threshold = request.POST['upper_threshold']
        obj.save()
        context ={
            'messages' : ["Settings updated successfully"],
            'lower_threshold':obj.lower_threshold,
            'upper_threshold':obj.upper_threshold,
        }
